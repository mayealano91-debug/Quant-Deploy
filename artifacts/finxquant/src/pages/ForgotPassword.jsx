    ...
    <div className="fixed inset-0 flex items-center justify-center bg-background">
      <div className="w-full max-w-sm border border-border bg-card p-8 rounded font-mono">
        <div className="mb-6 text-center">
          ...
          <p className="text-[10px] text-muted-foreground mt-1 tracking-widest uppercase">Reset Password</p>
        </div>
        {sent ? (
          <div className="text-center space-y-3">
            <p className="text-xs text-green-400">Reset link sent to {email}</p>
            <Link to="/login" className="block text-[10px] text-muted-foreground hover:text-primary">Back to login</Link>
          </div>
        ) : (
